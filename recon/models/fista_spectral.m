function [xout, loss_list]=fista_spectral(input_image, psf, mask, opts)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
maxeig = max(max(abs(fft2(pad2d(psf)))))^2;
L = maxeig*2;
...
elseif strcmp(opts.denoise_method, 'both') == 1
    opts.denoise_method
    prox = @(x) 0.5*(max(x,0) +  denoise_bound_init(x, 2*lambda/L, l, u, [], parsin));
    loss = @ (err, x) norm(err,'fro')^2;
else
...
%Hfor = @(x)mask.*crop2d(real((ifftn(Hs.*fftn((x))))));
%Hadj = @(x)real((ifftn(Hs_conj.*fftn((x)))));

Hfor = @(x)mask.*crop2d(real((ifftn(Hs.*fftn((x))))));
Hadj = @(x)real((ifftn(Hs_conj.*fftn((pad2d(x.*mask))))));
